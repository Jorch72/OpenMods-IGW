...
// Do NOT annotate this class: it will simply confuse the scanner!
public abstract class OpenModsEventHandler {
	...
	// Unable to annotate this handler due to probable errors at runtime.
	protected void replaceVariableWithBlockStatus(final VariableRetrievalEvent event) {
		...
		CACHE_BLOCK.put(configId,
				cpw.mods.fml.common.registry.GameRegistry.findBlock(this.getModId(), configId) != null);
	...
	protected void replaceVariableWithConfigValue(final VariableRetrievalEvent event) {
		...
		try {
			...
			final Class<?> cConfig = Class.forName(this.getConfigClass());
	...
	private String getPrefix(final VariableType varType) {
		...
		return String.format(OM_VARIABLE_PREFIX_RAW, this.getModId(), varType.id());
	}
}
